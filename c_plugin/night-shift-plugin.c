...
static void button_clicked(GtkWidget *button, NightShiftPlugin *ns) {
  ...
  gchar *cmd = g_strdup_printf("%s --x %d --y %d --w %d --h %d", PYTHON_PLUGIN,
                               x, y, width, height);
...
static void night_shift_free(XfcePanelPlugin *plugin, NightShiftPlugin *ns) {
  g_free(ns);
}

static gboolean size_changed(XfcePanelPlugin *plugin, gint size,
                             NightShiftPlugin *ns) {
  /* Adjust icon size based on panel size (subtract more for padding) */
  gint icon_size = size - 14;
  if (icon_size < 12)
    icon_size = 8;
...
static void night_shift_construct(XfcePanelPlugin *plugin) {
  ...
  /* Create the icon */
  ns->image = gtk_image_new_from_icon_name("night-light-symbolic",
  ...
  /* Connect signals */
  g_signal_connect(ns->button, "clicked", G_CALLBACK(button_clicked), ns);

  g_signal_connect(plugin, "free-data", G_CALLBACK(night_shift_free), ns);

  g_signal_connect(plugin, "size-changed", G_CALLBACK(size_changed), ns);
}
